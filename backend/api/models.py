...
class AgentModel(models.Model):
    from genaitor.core import AgentRole
    name = models.CharField(max_length=100, unique=True)
    role = models.CharField(max_length=50, choices=[(tag.name, tag.value) for tag in AgentRole], default=AgentRole.MAIN.name)
    tasks = models.ManyToManyField('api.TaskModel', related_name='agents')
    config = models.JSONField(default=dict, blank=True)
    conversation_history = models.JSONField(default=list, blank=True)
    task_history = models.JSONField(default=list, blank=True)
    ...
    llm_provider = models.CharField(max_length=100, null=True, blank=True)
